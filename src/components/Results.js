...
export default function Results({ filteredRange, balance }) {
    ...
    return (_jsxs("div", { className: "grid grid-cols-3 gap-4\t", children: [totalprofit !== 0.00 &&
                _jsxs("h1", { className: "text-3xl mt-4 bg-teal-50 shadow-md col-span-full\t\trounded-lg px-6 pt-5 py-3", children: ["Total Profit: \u00A0", _jsxs("span", { style: { color: parseFloat(totalprofit) >= parseFloat(balance) ? 'green' : 'red' }, children: ["$", parseFloat(totalprofit).toLocaleString(undefined, {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                })] })] }), console.log(parseFloat(balance)), _jsx(ProfitsBoard, { data: givendata }), _jsx("div", { className: "bg-teal-50 shadow-md rounded-lg py-4 flex justify-center items-center md:row-span-1 md:col-start-3 md:col-end-4 col-span-3", children: piechartdata.length > 0 && (_jsx("div", { className: " ", children: _jsx(ResponsiveContainer, { width: 200, height: 200, children: _jsxs(PieChart, { children: [_jsx(Pie, { data: piechartdata, cx: "50%", cy: "50%", labelLine: false, fill: "#8884d8", innerRadius: 50, outerRadius: 80, dataKey: "value", label: ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index, }) => {
                                        const radius = innerRadius + (outerRadius - innerRadius) * 0.5;
                                        const x = cx + radius * Math.cos(-midAngle * RADIAN);
                                        const y = cy + radius * Math.sin(-midAngle * RADIAN);
                                        return (_jsx("text", { x: x, y: y, fill: "black", textAnchor: x > cx ? 'start' : 'end', dominantBaseline: "middle", children: `${(percent * 100).toFixed(0)}%` }));
                                    }, children: piechartdata.map((entry, index) => (_jsx(Cell, { fill: Piecolors[index] }, `cell-${index}`))) }), _jsx(Legend, { verticalAlign: "bottom", height: 36 }), _jsx(Tooltip, {})] }) }) })) }), Object.keys(data).length > 0 && (_jsx("div", { className: "bg-teal-50 shadow-md rounded-lg col-span-full flex justify-center items-center", children: _jsxs(LineChart, { width: 700, height: 400, data: data, margin: {
                        top: 20,
                        right: 50,
                        left: 40,
                        bottom: 5,
                    }, children: [_jsx(XAxis, { dataKey: "date", tick: { fill: 'black' } }), _jsx(YAxis, { tick: { fill: 'black' }, children: _jsx(Label, { value: "Prices", position: "insideLeft", angle: 0, style: { textAnchor: 'middle', fill: 'black' }, offset: -10 }) }), _jsx(Tooltip, { formatter: (value) => `$${value}` }), _jsx(Legend, { height: 24 }), Object.keys(data[0])
                            .filter((key) => key !== 'date')
                            .map((entry, index) => (_jsx(Line, { type: "monotone", dataKey: entry, stroke: Piecolors[index], activeDot: { r: 10 } }, index)))] }) }))] }));
}
